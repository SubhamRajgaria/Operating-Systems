...
vector<int> fifo;
vector<int> lru;
int lru_last_replace;

long long int noOfPageFaults;
unsigned long long int executionTime;
long long int noOfPageTransfers;
...
int nru_policy(){
	int count =0;
	for(int i=lru_last_replace; count < NO_OF_PAGES;i=(i+1)%NO_OF_PAGES){
...
void printPageTable(){
	...
	for(i=0;i<NO_OF_PAGES;i++){
		cout << setw(10) << i << setw(40) << virtualmem[i] << setw(10) << extractFrame(virtualmem[i]) << "\n";
	}
}


int main(){
	int nframes, policy, lru_reset;
	srand(time(NULL));
	char *fileName = (char *)("instr.txt");
	char *token, *token2 ;
	char *delim = (char *)"\n", *delim2 = (char *)" ";
	...
	//cout << buffer << endl;
	num_of_set_pages =0, lru_last_replace=0;
	//}
	int j,k, trace;
	...
	if(policy==4){
		cout << "Enter 1 for resetting after every 1000 memory references else enter 0 to reset at 10th page fault" << endl;
		cin >> lru_reset;
	}
	for (int i=0; i< instructions.size(); i++){
		...
		if(policy==3){// For LRU putting recently used pages at the back in the queue
			...
			if(lru_reset==1){
				...
				if((noOfPageFaults%10)==0){ // Setting referenced bit 0 at every 10 page faults
					for(int c =0; c<NO_OF_PAGES;c++){
						virtualmem[c][31]=0;
					}
				}
	...
	long long int mult = (long)(noOfPageTransfers*3000);
	printf("%lld\n", mult);
	executionTime+= (long)(noOfPageTransfers*3000); // Every time page transfer happens 3000 memory cycles used
	printStatistics();
	printPageTable();
	return 0;
}
